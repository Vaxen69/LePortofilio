...
const SoftCool = ({ lang = 'fr', setLang = () => {} }) => {
        ...
        <div ref={railRef} style={{ position: 'relative', paddingLeft: railX + 28 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {entries.map((it, i) => {
              ...
              return (
                <Reveal key={`${it.kind}-${i}`} variant="up" delay={i * 70} style={{ position: 'relative', paddingBottom: 40 }}>
                  {/* Period */}
                  <div style={{ position: 'absolute', left: -(railX + 28), top: 4, width: railX - 14, textAlign: 'right', ...styles.mono, color: '#94a3b8', fontSize: 11, letterSpacing: '0.06em', lineHeight: 1.4 }}>
                    {(it.period || '').replace(' — ', '\n').split('\n').map((p, pi) => <span key={pi} style={{ display: 'block' }}>{p}</span>)}
                  </div>
                  {/* Dot */}
                  <span aria-hidden style={{ position: 'absolute', left: -33, top: 6, width: 12, height: 12, borderRadius: '50%', background: isWork ? '#fff' : accent, border: `2px solid ${accent}`, boxShadow: `0 0 0 3px ${accent}18` }}></span>
                  ...
                  {isWork ? (
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 5 }}>
                        {it.logo && (
                          <div style={{ width: 30, height: 30, borderRadius: 7, overflow: 'hidden', background: '#f1f5f9', border: '1px solid rgba(15,23,42,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, padding: fullBleed ? 0 : 4 }}>
                            <img src={it.logo} alt="" style={{ maxWidth: '100%', maxHeight: '100%', objectFit: fullBleed ? 'cover' : 'contain' }} />
                          </div>
                        )}
                        <span style={{ fontSize: 21, fontWeight: 700, color: '#0f172a', letterSpacing: '-0.02em', lineHeight: 1.15 }}>
                          {it.url ? <a href={it.url} target="_blank" rel="noopener noreferrer" style={{ color: 'inherit', textDecoration: 'none', transition: 'color .15s' }} onMouseEnter={(e) => e.currentTarget.style.color=accent} onMouseLeave={(e) => e.currentTarget.style.color='inherit'}>{it.org}</a> : it.org}
                        </span>
                      </div>
                      {/* Role */}
                      <div style={{ fontSize: 13, color: accent, fontWeight: 600, marginBottom: 12, letterSpacing: '0.01em' }}>{it.role}</div>
                      {it.bullets && it.bullets[0] && (
                        <p style={{ fontSize: 14, color: '#475569', lineHeight: 1.75, margin: '0 0 10px', maxWidth: 560 }}>{it.bullets[0]}</p>
                      )}
                      {/* Project buttons */}
                      {it.projectIds && it.projectIds.length > 0 && (
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
                          {it.projectIds.map((pid) => {
                            const proj = c.projects.items.find((p) => p.id === pid);
                            if (!proj) return null;
                            const label = proj.name.split(/\s[—·]\s/)[0];
                            return (
                              <button key={pid} onClick={() => setOpenProject(pid)} style={{ background: 'none', border: `1px solid ${accent}`, borderRadius: 8, cursor: 'pointer', color: accent, fontSize: 11, fontWeight: 500, padding: '4px 10px', display: 'inline-flex', alignItems: 'center', gap: 4, fontFamily: 'inherit', transition: 'background .15s, color .15s' }} onMouseEnter={(e) => { e.currentTarget.style.background=accent; e.currentTarget.style.color='#fff'; }} onMouseLeave={(e) => { e.currentTarget.style.background='none'; e.currentTarget.style.color=accent; }}>
                                {label} →
                              </button>
                            );
                          })}
                    ...
                    <div>
                      {/* Degree */}
                      <div style={{ fontSize: 20, fontWeight: 700, color: '#0f172a', letterSpacing: '-0.02em', lineHeight: 1.2, marginBottom: 4 }}>{it.degree}</div>
                      {/* School with small logo */}
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                        {it.logo && (
                          <div style={{ width: 22, height: 22, borderRadius: 5, overflow: 'hidden', background: '#f1f5f9', border: '1px solid rgba(15,23,42,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                            <img src={it.logo} alt="" style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'cover' }} />
                          </div>
                        )}
                        <span style={{ fontSize: 13, color: '#1d4ed8', fontWeight: 600 }}>
                          {it.url ? <a href={it.url} target="_blank" rel="noopener noreferrer" style={{ color: 'inherit', textDecoration: 'none', transition: 'opacity .15s' }} onMouseEnter={(e) => e.currentTarget.style.opacity='0.7'} onMouseLeave={(e) => e.currentTarget.style.opacity='1'}>{it.school}</a> : it.school}
                        </span>
                      </div>
                      {it.bullets && it.bullets[0] && (
                        <p style={{ fontSize: 14, color: '#475569', lineHeight: 1.75, margin: 0, maxWidth: 520 }}>{it.bullets[0]}</p>
                      )}
                    </div>
                  )}
                </Reveal>
        ...
        {items.map((p, i) => {
          ...
          return (
            ...
            >
              {/* Visual */}
              <div style={{
                height: isFeatured ? 240 : (p.video ? 220 : 170),
                ...
                {!isFeatured && p.video && (
                  <video src={p.video} muted loop playsInline style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'top center' }} onMouseEnter={(e) => e.currentTarget.play()} onMouseLeave={(e) => { e.currentTarget.pause(); e.currentTarget.currentTime = 0; }} />
                )}
  ...
  const SkillGrid = ({ items, col }) => {
    const ref = React.useRef(null);
    const [go, setGo] = React.useState(false);
    React.useEffect(() => {
      const el = ref.current;
      if (!el) return;
      const io = new IntersectionObserver(([e]) => { if (e.isIntersecting) { setGo(true); io.disconnect(); } }, { threshold: 0.15 });
      io.observe(el);
      return () => io.disconnect();
    }, []);
    return (
      <div ref={ref} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(110px, 1fr))', gap: '22px 8px' }}>
        {items.map((name, si) => {
          const logo = skillLogos[name];
          return (
            <div key={name} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', gap: 10, padding: '4px 6px', opacity: 0, animation: go ? `skillPop 0.45s cubic-bezier(0.34,1.56,0.64,1) both ${si * 60}ms` : 'none' }}>
              {logo ? (
  ...
  const Skills = () => {
    return (
      <Section id={c.nav.skills} eyebrow={lang === 'fr' ? '· Stack' : '· Stack'} title={c.skills.title} variant="up">
        <style>{`@keyframes skillPop { from { opacity: 0; transform: translateY(14px) scale(0.82); } to { opacity: 1; transform: translateY(0) scale(1); } }`}</style>
            ...
            return (
              <Reveal key={g.label} variant="up" delay={gi * 80}>
                <div style={{ ...styles.card, padding: '46px 36px 32px', position: 'relative', overflow: 'visible' }}>
                  ...
                  <SkillGrid items={g.items} col={col} />
                </div>
              </Reveal>
            );
          })}
